...
vxlan_interface_address=$(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $container_id)
...
consul services register -name $container_name -address $vxlan_interface_address -port $port

# Service check - Vérification de l'état du service
consul services register -name ${container_name}-check -address $vxlan_interface_address -port $port -check "docker ps | grep $container_id"
