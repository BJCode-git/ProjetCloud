...
vxlan_interface_address=$(consul catalog services | grep $service | awk '{print $2}')
...
if [ -z "$vxlan_interface_address" ]; then
...
docker run -d --name service1 -e CELERY_BROKER_URL="$service://$vxlan_interface_address:$port/0" -e CELERY_RESULT_BACKEND="redis://$vxlan_interface_address:$port/1" <image1>
docker run -d --name service2 -e CELERY_BROKER_URL="$service://$vxlan_interface_address:$port/0" -e CELERY_RESULT_BACKEND="redis://$vxlan_interface_address:$port/1" <image2>
